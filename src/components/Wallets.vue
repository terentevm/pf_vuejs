  ...
  <v-container id="scroll-target">
    <v-layout wrap>
      <v-flex xs12 sm12 md12 lg12>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container grid-list-md>
                ...
                <div class="row">
                  <div class="col-6 col-sm-6 col-md-6 col-lg-6">
                    <v-text-field
                      ...
                      row-height="15"
                      v-show="editedItem.is_creditcard == 1"
                    ></v-text-field>
                  </div>
                ...
                <p
                  class="title text-xs-left  balancePositive"
                  v-if="editedItem.currentBalance >= 0 && editedItem.id != null"
                >
                  <span
                    >Balance:
                    <v-btn flat @click="openChangeBalance()"
                      ><span class="title text-xs-left  balancePositive">{{
                        editedItem.currentBalance
                      }}</span>
                      <v-icon right green darken-3>edit</v-icon></v-btn
                    ></span
                  >
                </p>
                <p
                  class="title text-sm-left balanceNegative"
                  v-if="editedItem.currentBalance < 0 && editedItem.id != null"
                >
                  <span
                    >Balance:
                    <v-btn flat @click="openChangeBalance()"
                      ><span class="title text-xs-left  balanceNegative">{{
                        editedItem.currentBalance
                      }}</span>
                      <v-icon right green darken-3>edit</v-icon></v-btn
                    ></span
        ...
        <v-dialog persistent v-model="dialogCB" max-width="500px">
          <!-- Change balance -->
          <v-card>
            <v-card-text>
              <v-text-field
                autofocus
                label="New balance"
                value="editedItem.newBalance"
                v-model="editedItem.newBalance"
                type="number"
                row-height="15"
              ></v-text-field>
            </v-card-text>
