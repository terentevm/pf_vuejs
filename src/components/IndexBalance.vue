<template>
  <v-card>
    <v-toolbar color="yellow darken-3" dark>
      <v-toolbar-title><h3 class="font-weight-thin">Balance</h3></v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn icon>
    ...
    ></v-progress-circular>
    <v-list>
      <v-list-tile v-for="item in items" :key="item.wallet_id" avatar @click="">
        <v-list-tile-avatar>
        ...
        <v-list-tile-action>
          ...
          <v-list-tile-sub-title
            >{{ item.balance }} ({{ item.currencyCharCode }})</v-list-tile-sub-title
          >
        </v-list-tile-action>
      </v-list-tile>
    </v-list>
